...
# Create a smaller sample (e.g., 30% of the original dataset)
sample_size <- 0.3
sample_indices <- sample(1:nrow(your_data_frame_encoded), sample_size * nrow(your_data_frame_encoded))
your_data_frame_sample <- your_data_frame_encoded[sample_indices, ]
...
# Randomly generate values for mtry
tuneLength <- 15  # Increase tuneLength
tunegrid <- expand.grid(
  .mtry = sample(1:20, tuneLength)
)``

# Perform random search with repeated cross-validation on the smaller sample
rf_random_search <- train(
  Survived_1_year ~ .,
  data = your_data_frame_sample,
  method = 'rf',
  metric = 'Accuracy',
  tuneGrid = tunegrid,
  trControl = control
